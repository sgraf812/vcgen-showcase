...
| `EditDistance` | Levenshtein distance over `String`/`String.Pos`, two-row DP | rows `Array Nat` indexed by `Pos.offset.byteIdx`, both strings walked backward by `Pos.prev` so suffixes grow by cons, byte offset as the loop variant, a reusable position kit (`tailChars`, order↔`byteIdx` bridge) |
